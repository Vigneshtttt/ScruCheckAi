...
def index():
    ...
    if request.method == 'POST':
        ...
        if sample_file and prev_file and syllabus_file:
            ...
            # Content validation
            if len(sample_text) < 100 or len(prev_text) < 100 or len(syllabus_text) < 50:
                analysis_result = markdown2.markdown(
                    "**Error:** Insufficient content in uploaded files. "
                    "Please ensure files contain actual question papers and syllabus."
                )
            else:
